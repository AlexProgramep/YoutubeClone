...
def search(request):
    ...
    try:
        all_videos = VideoPost.objects.filter(title=query)
        user_obj = User.objects.filter(username__icontains=query)
    except:
        all_videos = VideoPost.objects.all().order_by('-id')
        user_obj = User.objects.none()
    params = {'all_videos': all_videos, 'user_obj': user_obj}
...
def watch_video(request, video_id):
    ...
    try:
        ...
        messages.warning(request, 'Зареєструйтеся для перегляду відео.')
        return redirect('home')

    session_object = User.objects.get(username=video_obj.user)
    profile_data = UserData.objects.get(user=session_object)
    ...
    params = {'video': video_obj, 'comments': video_comments, 'is_liked': is_liked, 'is_disliked': is_disliked,
              "user_data": profile_data, 'session_obj': session_obj, 'session_object': session_object}
...
def add_dislike(request, video_id):
    ...
    if user_obj in video_obj.dislikes.all():
        ...
        is_disliked = False
    return JsonResponse({'is_disliked': is_disliked, 'dislikes_count': video_obj.dislikes.all().count()})
...
def profile(request, session_username):
    ...
    video_cat_games = VideoPost.objects.filter(user=session_obj, category='Games').order_by('-id')
    ...
    params = {'subscribed': subscribed, 'session_obj': session_obj, 'user_data': profile_data, 'videos': user_posts,
              ...
              'edu': video_cat_education, 'food': video_cat_food, 'games': video_cat_games}
...
def dashboard(request, session_username):
    ...
    params = {'videos': user_videos, 'user_data': user_data, 'total_likes': user_video_likes,
              'total_dislikes': user_video_dislikes, 'total_views': user_videos_views}
...
def upload_video(request):
    if request.method == 'POST':
        ...
        upload_video = VideoPost(user=user_obj, title=title, desc=desc, video_file=video_file, thumbnail=thumb_nail,
                                 category=cate)
        upload_video.save()
        messages.success(request, 'Відео завантажено.')
...
def delete_video(request):
    if request.method == 'GET':
        ...
        return JsonResponse({'video_id': vid, 'videosCount': user_videos.count(), 'videosLikes': user_video_likes,
                             'videosDisLikes': user_video_dislikes})
    else:
    ...
    if not request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if check_user is not None:
                ...
                messages.warning(request, "Неправильне ім'я користувача або пароль.")
                return redirect('home')
        return redirect('home')
